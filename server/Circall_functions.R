...
# 04 Sep 2020: fix the problem that a tx in gtf but not in cdna in getCircPseudoSequence_cDNA and getTandemSequence_cDNA
...
SE_filtering <- function(inPath, gtfSqlite, outFn = NULL, read_mapProp = 0.99, read_anchorLen = 10, eqc_txMinProp = 1, eqc_minCount = 1, hard_hitlen = 50){
  ...
  read_dat=NULL
  for (fn in fileList){
  a_read_dat = read.table(fn, sep = "\t", header = FALSE, stringsAsFactors = FALSE)
...
load_read2_2_file <- function(path_to_header, path_to_mapInfo){
  Txname = read.delim(path_to_mapInfo, stringsAsFactors = FALSE, header=FALSE)
  Header = read.delim(path_to_header, stringsAsFactors = FALSE, header=FALSE)
...
getCircPseudoSequence_cDNA <-function (myexonInfo, fasta_genome=NULL,tx.all.fasta=NULL, tx.all.NAME=NULL, circRNA_name, start, end, num=-1,readLen){
  ...
  if (txNum > 0){ # if existing transcripts containing both EXONSTART and EXONEND
    ...
    if (nrow(myex) >0){ #19July2022/Nghia: fix the case non-coding circRNA
