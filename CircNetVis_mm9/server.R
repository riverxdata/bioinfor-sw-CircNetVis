...
#setwd("C:\\Nghiavtr\\Workplaces\\Projects\\circularRNAs\\CircNetVis_mm9\\")
...
#load targetscan
load("Annotation_data/targetscan_mmu_db_v72.RData")
load("Annotation_data/mmu_circRNA_mirRNA_annotation_processed_small.RData")
#load("Annotation_data/circRNA_RBP.RData") ###  RBP_dat_NET: "circRNA" "RBP"     "SiteNum" "Line"

#goa=fread("Annotation_data/goa_human_isoform.gaf.gz", skip = 27
goa=readRDS("Annotation_data/goa_human_isoform.gaf.RDS")
...
getcircinfo = function(input) {
  ...
  res = eventReactive(input$goButton, {
    circRNA.in = parsingInput(input$circRNAinput, mmuCirData_small, intype =
                                "circRNA")
    
    if (length(circRNA.in) > 0){
      circRNAIDlist_res<-processcircRNAIDlist(circRNA.in,mmuCirData_small,mmuMirData_small)
      out = list(
...
#   circRNA.in = parsingInput(input$circRNAinput, mmuCirData_small, intype = "cirRNA")
...
#   if (length(circRNA.in) > 0) {
#     pick1 = which (
#       mmuCirData_small$normID %in% circRNA.in |
#         mmuCirData_small$circRNA.ID %in% circRNA.in
#     )
#     mmuCirData_small = mmuCirData_small[pick1,]
#     pick2 = which (RBP_dat_NET$circRNA %in% mmuCirData_small$circRNA.ID)
...
#       CircRBP_int.net$normID = mmuCirData_small$normID[match(CircRBP_int.net$circRNA, mmuCirData_small$circRNA.ID)]
#     }
#   }
  ...
  function(input, output, session) {
    ...
    mynetwork <- reactive({
      ...
      if (!is.null(circInfo)) {
        ...
        pick = targetscan_mmu_db$miRNA %in% miRNAset
        migen.net = targetscan_mmu_db[pick, ]
        ...
        RNAhybrid_cirmi.net.info=RNAhybrid_cirmi.net[match(cirmi.net$int,RNAhybrid_cirmi.net$int),]
        miRanda_cirmi.net.info=miRanda_cirmi.net[match(cirmi.net$int,miRanda_cirmi.net$int),]
        TargetScan_cirmi.net.info=TargetScan_cirmi.net[match(cirmi.net$int,TargetScan_cirmi.net$int),]
        
        cirmi.net.info=NULL
        for (i in 1:nrow(cirmi.net)){
          x=TargetScan_cirmi.net.info[i,c("st_6mer", "st_7mer1a", "st_7merm8","st_8mer1a")]
          names(x)=gsub("st_","",names(x))
          x=paste0(names(x),"=",x)
          x=paste(x,collapse = ";")
          ts=x
          
          x=RNAhybrid_cirmi.net.info[i,c("mfe","pval")]
          x$pval=round(x$pval, )
          x=paste0(names(x),"=",x)
          x=paste(x,collapse = ";")
          rhb=x
          
          x=as.integer(miRanda_cirmi.net.info[i,c("Max_Score")])
          x=paste0("Max_Score=",x,collapse = ";")
          mr=x
          
          x=paste(c(ts,rhb,mr),collapse = ";")
          
          cirmi.net.info=c(cirmi.net.info,x)
        }
        #cirmi.net.info
        migen.net.info=NULL
        for (i in 1:nrow(migen.net)){
          x=migen.net[i,c("weighted.context...score", "weighted.context...score.percentile")]
          #names(x)=gsub("\\.\\.\\.","",names(x))
          names(x)=c("score","score.percentile")
          x=paste0(names(x),"=",x)
          x=paste(x,collapse = ";")
          migen.net.info=c(migen.net.info,x)
        }
        ...
        migen.net.info = migen.net.info[selectID]
        ...
        if (input$cbShowcircbaseID_cirmi) {
          m = match(nodecir_show, mmuCirData_small$normID)
          p1 = which(!is.na(m))
          if (length(p1) > 0)
            nodecir_show[p1] = mmuCirData_small$circRNA.ID[m[p1]]
        }
        ...
        edges.to = c(cirmi.to)
        edges.title=cirmi.net.info
        edges.dashes = rep(FALSE, length(cirmi.net.info))
        ...
        if (length(nodegen) > 0) {
          ...
          edges.title=c(edges.title,migen.net.info)
          edges.dashes=c(edges.dashes,rep(TRUE, length(migen.net.info)))
        }
...
#           # CircRBP_int.net$normID = mmuCirData_small$normID[match(CircRBP_int.net$circRNA, mmuCirData_small$circRNA.ID)] #### Adding normID
...
#   
    ...
    getIntTables <- function() {
      ...
      m = match(edges_from, mmuCirData_small$circRNA.ID)
      p1 = which(!is.na(m))
      if (length(p1) > 0)
        #edges_from[p1] = mmuCirData_small$normID[m[p1]]
        edges_from = c(edges_from,mmuCirData_small$normID[m[p1]])
      ...
      m = match(cirmi.net$normID, mmuCirData_small$normID)
      p1 = which(!is.na(m))
      if (length(p1) > 0) cirmi.net$int[p1] = mmuCirData_small$circRNA.ID[m[p1]] else cirmi.net$int=cirmi.net$normID
      colnames(cirmi.net)[1] = "circRNA"
      x=colnames(cirmi.net)
      x=c(setdiff(x,"normID"),"normID") #put normID to the end
      cirmi.net=cirmi.net[,x]

      m = match(cirmi.net$circRNA, mmuCirData_small$circRNA.ID)
      p1 = which(!is.na(m))
      if (length(p1) > 0) cirmi.net$normID[p1] = mmuCirData_small$normID[m[p1]]
    ...
    ##########
    observeEvent(input$inputSpecies, {
      if (input$inputSpecies=="Human - hg19")
        #browseURL("https://www.meb.ki.se/shiny/truvu/CircNetVis/", browser = "false")
#      
        showModal(modalDialog(title="You selected Human - hg19", tags$a(href = "https://www.meb.ki.se/shiny/truvu/CircNetVis/","Click here to move")))
        updateSelectInput(session, "inputSpecies", selected  = "Mouse - mm9")
    })
    ...
    output$goAnno <- renderDataTable(server = FALSE,{
      geneset = NULL
      if (input$rbgset == "net") {
        if (!is.null(input$circRNAnetwork_nodes)) {
          mynodes = input$circRNAnetwork_nodes
          ...
          pick = which(res_group == "gene")
          geneset = res_label[pick]
          #geneset = paste(geneset, collapse = " ") #just use for GSEA
        }
      } else{
        #NO, should from the network, not all genes
        #get all genes
      ...
      if (is.null (geneset)) {
        ...
        if (trimws(geneset_str) == "")
          return(DT::datatable(matrix(0, 1, 1)))
        ### get GOA from database and save to out
        geneset=trimws(geneset)
        geneset=toupper(geneset)
        p=which(goa$GeneSymbol %in% geneset)
        out=goa[p,]

        #creating hyperlink of GO to reactome page
        out$GO<-
          sapply(out$GO, function(x)
            toString(tags$a(
              href = paste0('https://amigo.geneontology.org/amigo/term/', x), x
            )))
        
        geneQual=paste0(out$GeneSymbol,out$Qualifier)
        outGO=tapply(out$GO,geneQual,c)
        outGO2=sapply(outGO,function(x) paste(x, sep=","))
        m=match(geneQual,names(outGO2))
        out$GO=outGO2[m]
        d=duplicated(geneQual)
        out=out[which(!d),]
        o=order(nchar(out$GO),decreasing = FALSE)
        out=out[o,]
        rownames(out)=NULL
