...
##############################
processcircRNAIDlist <-function(circRNA.in,mmuCirData_small,mmuMirData_small){
    ...
    #normID_in="5__102210394__102227535"
    p = which(mmuCirData_small$normID %in% normID_in)
    if (length(p) > 0) {
      # if existing in the database
      index_in = mmuCirData_small$index[p]
      
      ### collect results from RNAhybrid
      #connect to database
      con <- dbConnect(RSQLite::SQLite(), "RNAhybrid_cirmi_mm9.db")
      
      cirmi = lapply(index_in, function(x) {
        y = dbGetQuery(con,
                       paste(
                         "select * from RNAhybrid_cirmi_mm9 where normID='",
                         x,
                         "'",
                         sep = ""
                       ))
      ...
      cirmi$normID = mmuCirData_small$normID[match(cirmi$normID, mmuCirData_small$index)]
      cirmi$miRNA = mmuMirData_small$miRNA[match(cirmi$miRNA, mmuMirData_small$index)]
...
#    cat("\n RNAhybrid_cirmi.net ", dim(RNAhybrid_cirmi.net))
    ...
    if (length(p) > 0) {
      ...
      #connect to database
      con2 <- dbConnect(RSQLite::SQLite(), "miRanda_cirmi_mm9.db")
      
      miRanda_cirmi = lapply(index_in, function(x) {
        y = dbGetQuery(con2,
                       paste(
                         "select * from miRanda_cirmi_mm9 where normID='",
                         x,
                         "'",
                         sep = ""
                       ))
      ...
      miRanda_cirmi$normID = mmuCirData_small$normID[match(miRanda_cirmi$normID, mmuCirData_small$index)]
      miRanda_cirmi$miRNA = mmuMirData_small$miRNA[match(miRanda_cirmi$miRNA, mmuMirData_small$index)]
      ...
      if (nrow(miRanda_cirmi) > 0) {
        ...
        miRanda_cirmi.net$siteNum = as.double(miRanda_cirmi.net$siteNum)
...
#    cat("\n miRanda_cirmi.net ", dim(miRanda_cirmi.net))
    ...
    if (length(p) > 0) {
      ...
      #connect to database
      con3 <-
        dbConnect(RSQLite::SQLite(), "TargetScan_cirmi_mm9.db")
      
      TargetScan_cirmi = lapply(index_in, function(x) {
        y = dbGetQuery(
          con3,
          paste(
            "select * from TargetScan_cirmi_mm9 where normID='",
            x,
            "'",
            sep = ""
          )
      ...
      TargetScan_cirmi$normID = mmuCirData_small$normID[match(TargetScan_cirmi$normID, mmuCirData_small$index)]
      TargetScan_cirmi$miRNA = mmuMirData_small$miRNA[match(TargetScan_cirmi$miRNA, mmuMirData_small$index)]
