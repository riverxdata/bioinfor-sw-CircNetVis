...
runRNAhybrid = function(pseudoSeq.fa){
    ...
    miRNA_db="Annotation_data/miRNA_mature_mmu.fa"
    ...
    #cmd_RNAhybrid=paste("./Tools/RNAhybrid -m ",maxWidth," -n 29 -b 1 -c -p 1.0 -q ",miRNA_db," -t ",pseudoSeq_fn, " > ",circRNA_miRNA_out,sep="") ## we run for mouse, but there is no data for mouse. If do not use -s then we need to put value for -d, but we dont know which one we should put in Therefore, we use human data instead.
...
runTargetScan = function(pseudoSeq.fa){
  ...
  miRNA_file="Annotation_data/mmumiR_seed_full.txt"
...
getCirmi_RNAhybrid = function(circRNA.in){
  ...
  load("Annotation_data/mm9.genes.exon.all.RData")
...
getCircPseudoSequence_cDNA <-function (myexonInfo, fasta_genome=NULL,tx.all.fasta=NULL, tx.all.NAME=NULL, circRNA_name, start, end, num=-1,readLen){
  #load tx name
  load("Annotation_data/mmu_mm9.tx.NAME.RData")
  #output
  ...
  if (txNum > 0){ # if existing transcripts containing both EXONSTART and EXONEND
    ...
    #load data here
    if (is.null(tx.all.fasta)) load("Annotation_data/mmu_mm9.all.fa.RData")
...
#    x=substring(fasta_genome[chrID],startR,endR)
    if (is.null(fasta_genome)){
      suppressMessages(suppressWarnings(require(BSgenome.Mmusculus.UCSC.mm9)))
      fasta_genome<-BSgenome.Mmusculus.UCSC.mm9    
    ...
    #unload BSgenome.Mmusculus.UCSC.mm9
    detach(package:BSgenome.Mmusculus.UCSC.mm9)
